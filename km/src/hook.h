...
namespace hook {
	bool call_kernel_function(void* kernel_function_address) {
		...
		PVOID* func = reinterpret_cast<PVOID*>(memory::get_system_module_export("\\SystemRoot\\System32\\drivers\\dxgkrnl.sys", "NtOpenCompositionSurfaceSectionInfo"));
		if (!func)
			return false;
		...
		memory::write_to_read_only_memory(func, &orig, sizeof(orig));
